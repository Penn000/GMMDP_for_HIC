...
def getIndianSpectral():
    ...
    for i in range(145):
        for j in range(145):
            if indianDataY[i][j] in selected_class:
                ...
                # vec = np.array(indianDataX[i, j, :])*1.0
                spectral.append(vec)
    ...
    return np.array(spectral), np.array(y)
...
def getIndianDataset(spatialFeature):
    ...
    if spatialFeature == 'neighbor':
        ...
        if os.path.exists(indianNpyfileWaveletSpatial):
            X_spatial = np.load(indianNpyfileWaveletSpatial)
        else:
